 ...
 *
 * Result:
 * Runtime: 17 ms, faster than 70.35% of Java online submissions
 * Memory Usage: 93 MB, less than 11.81% of Java online submissions
 */
...
public class Solution {
    public int maxWidthOfVerticalArea(int[][] points) {
        ...
        for (int i = xCoordinates.length - 1; i > 0; i--) {
            max = Math.max(max, (xCoordinates[i] - xCoordinates[i - 1]));
        }
        return max;
    }
}
