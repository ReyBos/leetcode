 ...
 *
 * Result:
 * Runtime: 6 ms, faster than 46.71% of Java online submissions
 * Memory Usage: 39.8 MB, less than 43.35% of Java online submissions
 */
...
public class FirstSolution {
    public List<List<Integer>> groupThePeople(int[] groupSizes) {
        ...
        for (int i = 0; i < groupSizes.length; i++) {
            ...
            grouped.get(groupSizes[i]).add(i);
        }
        for (Integer key : grouped.keySet()) {
            curGroup = grouped.get(key);
            if (curGroup.size() == key) {
                rsl.add(curGroup);
            } else {
                for (int i = 0; i < curGroup.size(); i += key) {
                    List<Integer> chunk = new ArrayList<>(curGroup.subList(i, Math.min(curGroup.size(), i + key)));
                    rsl.add(chunk);
                }
            }
        }
