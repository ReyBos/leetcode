 ...
 * Memory Usage: 39.2 MB, less than 60.06% of Java online submissions
 */

public class Solution {
    public int[] finalPrices(int[] prices) {
        for (int i = 0; i < prices.length - 1; i++) {
            prices[i] -= findDiscount(prices, (i + 1), prices[i]);
        }
        return prices;
    }

    private int findDiscount(int[] prices, int pointer, int currPrice) {
